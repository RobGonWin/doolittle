...
This ledger tracks the literal Doolittle gaps that still remain after the current gateway/API native experience wave. Items that are already present in Doolittle are called out briefly so the focus stays on the residual native experience debt.
...
| Doolittle claim | Doolittle references | Doolittle surfaces | Status | Remaining literal gap |
|---|---|---|---|---|
...
Doolittle now matches the broad gateway/API shape much more closely. The remaining work is less about basic architecture and more about literal native experience fidelity: daemon recovery, journal semantics, Responses transport behavior, and platform-native live control.
